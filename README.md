...
│   ├── pubsub_*_example.py  # PubSub SDK 예제
...
python src/pubsub_subscriber_example.py       # PubSub SDK (폴링)
python src/pubsub_subscriber_callback_example.py  # PubSub SDK (콜백)
```
...
python src/pubsub_publisher_example.py  # PubSub SDK
```

- `subscriber.py`: XTypes 동적 타입 발견 + WaitSet 폴링
- `subscriber_callback.py`: 정적 타입 + Listener 콜백
- `pubsub_*_example.py`: PubSub SDK 사용 (write/read, 예외 처리, close/with)
...
# 구독 (콜백)
subscriber = Subscriber("HelloWorld", datatype=HelloWorld, on_message=callback)
```

- 예제: `pubsub_publisher_example.py`, `pubsub_subscriber_example.py`, `pubsub_subscriber_callback_example.py`
