...
| **설치** | `./idls/idl_to_py.sh` 실행 시 `pip install -e .` 자동 수행 (프로젝트 루트) |
| **의존성** | `cyclonedds`만 필요 (RPC/PubSub SDK 및 IDL 메시지 타입 포함) |
...
- participant를 넘기면 domain_id는 무시되고, close 시 participant는 정리하지 않음.
...
stop() -> None  # 루프 종료
close() -> None # 리소스 정리 (stop 포함)
```

`run()`은 `stop()`이 호출되거나 KeyboardInterrupt까지 블로킹됩니다.  
`close()` 후 `run()`/`register_service()` 호출 시 `RPCClosedError` 발생.  
`with RpcServer(...) as server:` 사용 시 종료 시 `close()` 자동 호출.
...
- participant를 넘기면 domain_id는 무시됨. `close()` 시 participant는 정리하지 않음.
...
close() 후 call()/list_servers() 호출 시 `RPCClosedError`.  
`with RpcClient(...) as client:` 사용 시 종료 시 자동 호출.
...
| 예외 | 발생 조건 |
|------|-----------|
| `ConfigurationError` | 인자 오류 (domain_id < 0 등) |
| `RPCClosedError` | close() 후 call()/list_servers()/run()/register_service() 호출 시 |
