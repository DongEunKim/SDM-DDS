...
- `ReplyHeader`: `related_request_id`, `remote_ex`, `server_instance` (응답 서버 인스턴스명)
...
| 메서드 | 시그니처 | 설명 |
|--------|----------|------|
| `register_service` | `(name, RequestType, ResponseType, handler, instance_name=None) -> None` | 서비스 등록 |
| `run` | `() -> None` | 이벤트 루프 실행 (블로킹) |
| `stop` | `() -> None` | 루프 종료 (선택) |

- `handler`: `(request: RequestType) -> ResponseType` 콜러블
- `instance_name`: 지정 시 해당 인스턴스로만 요청 처리. 중복 등록 시 `RPCDuplicateInstanceError`
- `instance_name` 지정 서버만 레지스트리에 등록되어 `list_servers()`로 조회 가능

### 4.2 RpcClient (클라이언트)

| 메서드 | 시그니처 | 설명 |
|--------|----------|------|
| `call` | `(name, request, ResponseType, timeout=5.0, instance=None) -> ResponseType` | 동기 호출 |
| `list_servers` | `(service_name=None, timeout=3.0) -> list[(str, str)]` | 서버 목록 조회 |

- `call`의 `instance`: 대상 서버 인스턴스명. None이면 아무 서버나 응답
- `response.header.server_instance`: 응답한 서버 인스턴스명
- `list_servers`: `[(service_name, instance_name), ...]` 반환
...
| `RPCDuplicateInstanceError` | 동일 서비스에 중복 인스턴스명 등록 시도 |
